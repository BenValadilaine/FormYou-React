...
const EditProfileForm = ({ }) => {
    ...
    const handleUpdateProfile = async (event) => {
        ...
        const passwordConfirmationValue = document.querySelector("#password_confirmation").value;
        const ageInputValue = document.querySelector("#age").value;

        if (firstNameInputValue != "" && lastNameInputValue != "" && emailInputValue != "" && passwordInputValue != "" && passwordConfirmationValue != "" && ageInputValue != "") {
            if (passwordInputValue == passwordConfirmationValue) {
                const jwt_token = Cookies.get('jwt_token');
                const user_id = jwt_decode(jwt_token).sub;


                const user_role = await API_REQUEST.find(API_ENDPOINTS["users"] + `/${user_id}/roles`, true, jwt_token);
                ...
                const response = await API_REQUEST.update(
                    ...
                    API_ENDPOINTS["users"] + `/${user_id}`, true, jwt_token
                ).then((e)=>e.json())
    ...
    return (
        <form action="" className="overflow-auto">
            ...
            <FormGroup label="Age:" inputName="age" inputId="age" inputType="number" />
            ...
            <button className="btn btn-scheme-2 btn-lg col-12 my-4" type="submit" onClick={handleUpdateProfile}>VALIDER</button>
